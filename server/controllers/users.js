...
router.get('/api/users/:username/:password', async function (req, res, next) {
    await Users.findOne({ 'username': req.params.username }, function (err, users) {
        if (err) { return next(err); }
        if (users === null) {
            return res.status(404).json({ "message": "User not found" });
        }
        if (users.username === req.params.username) {
            var scrambledPass = scramblePass(req.params.password);
            if (scrambledPass == users.password) {
